...
gpuid = "0,1"
NET_ID = 'RESNET-101'  # model name deeplab_largeFOV
MODEL = ''
DATA_ROOT = ''  # since we use absolute path


sys.path.insert(0, codes_path+'/DeepLab-Context2')
sys.path.insert(0,codes_path+'/DeepLab-Context2/python/my_script/')

os.environ['DEEPLAB'] = codes_path+'/DeepLab-Context2'
...
run_deeplab.MODEL = MODEL
run_deeplab.DATA_ROOT = DATA_ROOT
run_deeplab.NUM_LABELS = 2  # only building and background
...
def convert_mat_to_png(mat_folder, b_runmatlab=True):
    ...
    # path = os.path.join(codes_path,'DeepLab-Context2','matlab','my_script')
    # os.chdir(path)
